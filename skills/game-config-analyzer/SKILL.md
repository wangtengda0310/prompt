...
description: |
  游戏配表分析技能 — 只要用户的问题涉及 Excel 配置文件（.xlsx）的数据、结构、关系或内容，就必须使用此技能。

  **触发规则（满足任一条即触发）**：

  一、用户提到"配表"、"配置表"、"配置文件"、"Excel"、"xlsx"中的任何一个词，且在提问、搜索、查看或分析数据时。

  二、用户的问题匹配以下模式：
  - 「X 涉及哪些表/文件」「X 在哪个文件/表中配置」「X 跟哪些表有关」
  - 「搜索/查找/查一下 X 在哪些表中出现」「X 在哪里配置的」
  - 「分析/看一下/帮我看看 X 系统的配置」「X 系统怎么配的」
  - 「X 和 Y 之间是什么关系」「X 引用了哪些表」「哪些表依赖 X」
  - 「X 有什么用」「X 的作用」「X 是怎么工作的」
  ...
  四、即使用户只说了"分析一下 X 系统"、"搜索 X"、"查一下 X"，只要上下文暗示涉及配置数据，也应触发。

  **不要触发**：用户只是要求直接编辑/修改 Excel 文件内容（此时用 excel-parser 技能）。
...
**一旦本技能被触发，你必须严格按以下步骤执行，不允许跳过任何步骤：**

1. **扫描定位** — 确定配表目录，扫描相关文件（按文件名/关键词/表头内容）
2. **读取结构** — 用 openpyxl 读取目标表的完整字段（4行表头：中文名→类型→字段名→导出标识）
3. **交叉引用** — 追踪数据链路，找到表之间的关联（XxxId→Xxx.Id, Buff→Buff表, E#枚举→enum/目录）
4. **约束提取** — 提取字段级校验规则（必填、引用完整性、时间顺序、枚举有效性、格式合法性）
5. **文档输出** — 将结果整理为 Markdown 文档，保存到项目的 docs/ 目录

**为什么这很重要**：跳过步骤会导致分析不完整。例如只扫描不交叉引用，就会漏掉关键的数据依赖关系；只读结构不提取约束，就无法发现数据质量问题。这5步是经过实践验证的最小完整分析闭环。

**两种分析模式**：

- **全量扫描**（用户要求分析整个项目的配表）→ 先跑 `scripts/analyzer.py` 获取全局关系图，再逐系统深入
- **聚焦分析**（用户问某个具体系统/字段/值）→ 从目标表出发，增量发现关联表，逐步展开

## 常见游戏引用模式

分析时注意识别以下特殊格式，它们是游戏配表中常见的跨表引用方式：

| 模式 | 说明 | 示例 |
|------|------|------|
| `XxxId` → 同名表 | ID引用 | `SkillId` → Skill表, `HeroItemId` → Item表 |
| `{ID;数量}` | 奖励/物品格式，花括号内分号分隔 | `{1000005;1}{1000011;10}` |
| `E#枚举名` | 枚举类型，定义在 `enum/` 子目录 | `EWeather` → enum/EWeather_enum.xlsx |
| `ItemCfg[]` | 物品配置数组 | Reward字段 |
| `BuffXxx` | Buff枚举名，对应 Buff 表中 Id | `Buff=BuffMengHuXiaoLin` |
| Sheet名含 `中|英` | 如 `成就表|Achieve`，竖线后为英文名 | 匹配时用英文名部分 |
